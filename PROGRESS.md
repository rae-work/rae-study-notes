...
**当前状态：v1.9.1 已上线（2026-09-13，Rae 批准）。** 1.9.1 只改了一处：中文界面整站用系统无衬线（iOS 没有中文宋体，原来掉到日文明朝体混排）。
**四课 85 页 + 总目录页，450 词，五语（中 / 日 / 英 / 越 / 波），音频 100%。**

**上一版：v1.8.0 已上线（2026-09-07，Rae 批准）。**
**三课 67 页 + 总目录页（其中 16 页是课本练习的参考答案），366 词，
五语（中 / 日 / 英 / 越 / 波），音频 100%。**
...
## v1.9.0 做了什么（2026-09-13）

面向学习者的说明在 `CHANGELOG.md`，下面只记工程上要知道的。

| | |
| --- | --- |
| 来源 | `inbox/4/`：课本 Bab 4 照片 15 张（第 43–57 页，HEIC）+ 讲义 PDF「3. BAB 3 - Kalimat Sederhana」（跟 L03 用过的 docx 版几乎一样） |
| 第四课 | `L04.json` 17 页（生成脚本在会话 scratchpad，没进仓库）。课文 / 对话 / 语法例句仿写；**练习题用课本原句、按原题号**（Rae 2026-09-13 决定，已写进 CLAUDE.md） |
| 引擎 | `qa_list` 的印尼语答案可带 `hl: ["片段", …]`：按顺序找到的片段用 kw 样式加粗（`sayLineHl()`），有 hl 时不再标 kw。`validate.js` 第 1 关检查片段能按顺序找到。**先做过「保底版 / 完整版」两行，Rae 看了嫌重复、太乱，当天改成一句 + 粗体**，两行版的代码和 `block.qa_short/qa_full` 键已删 |
| 第三课 | 讲义 1b–4b 的 40 题换成更长的句子 + 粗体核心；新增第 26 页「Latihan Kalimat 3」（1a–4a 各 12 行示范表）；原「课本还剩一题」note 改为指向新页。L03 变 28 页 |
| 词汇 / 题库 | +84 词（`les: 4`，例句取课内句子）；`situasi` +6（家庭问答） |
| 语音 | 已合成 580 条 / 15,080 字符（Rae 批准，2026-09-13），覆盖率 100%，账号剩约 12,800 |
...
- **印刷版课本第 54 页对话漏了两行**（Ibu 和 Ayu 去奶奶家 / 奶奶病了吗），电子版 `inbox/done/L03/book.txt` 里有。对话页和练习页都说明了。
- 其他笔误：第 47 页答题线印「?」、第 49 页 Latihan 1 第 8 题漏人名、「danb」、「Adik Saya」、第 51 页 mahasiwa、第 52 页 Hiroko 例句问号 + 放错到形容词下面、第 54 页 Latihan 5 第 1 题句子没印完 / 第 8 题空格位置错、讲义 PDF 4b 题目写成 nomina+nomina。
- **`<s>` 里别放带省略号的片段和后缀**（`<s>Di …</s>`、`<s>-nya</s>`）—— 会各生成一条没意义的音频。这次在翻译回填时统一换成 `<b>`。
- 讲义（BAB 3 语法）和课本（Bab 4）是两条线：老师讲义编号跟课本章号不同步，讲义内容仍放在 L03。

## v1.8.0 做了什么（2026-09-07）

面向学习者的说明在 `CHANGELOG.md`，下面只记工程上要知道的。

| | |
| --- | --- |
| 起因 | Rae：「把所有书本上的练习题都写参考答案附在每一课里」，为了预习和复习 |
| 内容 | 新增 16 页 `cat: latihan` 的练习页（L01 +4、L02 +5、L03 +7），约 200 道题，每题带参考答案。51 → 67 页 |
| 引擎 | `qaSide()` 的印尼语侧现在认 `gloss`：释义放进 `.ans` 里，跟句子一起被遮挡。CSS 加了 `.qa-line .gloss` |
| 语音 | 新增 241 条 / 5,669 字符（Rae 确认过），覆盖率回到 100% |
| 材料出处 | `inbox/done/L03/book.txt`（Bab 1–3）、`docx-bab1/2/3.txt`（三份讲义）、`buku-bab3-foto/`（课本 23–41 页照片，book.txt 缺的那几页） |

**这一轮值得记的**

- **题目一律仿写，不照抄课本。** 课本的题型、语法点、提示词（Latihan 1–4 的
  `Ini / Itu / Saya / …` 那几列、`Alva / Kita / Besar / Bagus …`）照收 ——
  那些是语言事实；**句子全部自己写**。每页开头的 `lead` 都写明「题型照搬、句子另写，
  答案只是其中一种说法」，免得同学以为跟课本答案不一样就是错。
- **课本 Bab 3 的 A 部分（Menyimak）两题给不了答案** —— 它要教材自己的两段录音
  （`http://ugm.id/audiotitianbahasapemula1`），仓库里没有，L03 也没有 `listen` 块。
  「Latihan Menyimak dan Membaca」那页如实写明了这件事，改用本课课文
  （Shinta 的生日）出了一份同题型的 benar / salah 判断题。**别以后又去补这两题。**
- **写练习时踩到的三个坑**
  - `qa_list` 的提示词里**不要写斜杠**：`"Aku / saya"` 经 `sayText()` 会变成
    朗读文本 `"Aku saya"`，白花一条额度合成一句没意义的话。拆成一个词，
    另一个写进 `gloss` 或 `note`。
  - **后缀用 `<b>-ku</b>` 不要用 `<s>-ku</s>`** —— `<s>` 会让它进音频清单，
    单独合成一个 `-ku` 没有用。
  - **英 / 越 / 波文本里不许出现全角符号**（`＋`、`／`、`，`）：`speakables.js`
    第 5 关的渗漏检查按 Unicode 区段判断，`\uff00-\uffef` 会被当成中日文字符，
    直接构建失败。
- **写新句子前先看一眼 `content/vocab.json` 里该课的词。** 这一轮初稿在 L02 的
  范文里用了 `kamar`（L3 才教）和 `kecil`（词表里根本没有），后来改掉了。
  扫一遍的办法：把课文里的词跟 `vocab.json` 的 `les` 对一遍，出现得比本课晚的挑出来。

## v1.7.0 做了什么（2026-09-07）

面向学习者的说明在 `CHANGELOG.md`，下面只记工程上要知道的。

| | |
| --- | --- |
| 内容 | 全部 2,038 个多语对象补上 `pl`（三课 + 词汇 + 题库 + 标题），界面文案 `ui.pl.json` 199 键 |
| 引擎 / 脚本 | `TRI_LANGS` 加 `pl`；`speakables.js` 的渗漏检查加跑一遍波兰语界面；`LZ()` 改成**通用占位符**（见下）；`validate.js` 第 2 关新增「占位符自洽」检查 |
| meta.json | `langs` / `required_langs` 加 `pl`，`lang_names.pl = Polski`，`learner.pl = { negara: Polandia, nama: Polska, nama_gen: Polski, people: Polakiem }` |
| 界面 | **语言选择从一排芯片改成原生 `<select>`**（五种语言排一行会顶破 246px 的设置面板）。发音语音那个下拉一起改用 `.sel-ctl`，两个都补了下拉小箭头 |
| 语音 | 波兰语界面的 `Polandia` 句子新增 12 条 / 274 字符，已合成，覆盖率回到 100% |
| 顺手修 | L03「时间词」表的波兰语表头太长，375px 下要横拖 —— 改成 `Okres / Minione · lalu / Teraz · ini / Przyszłe · depan` |

**这一轮值得记的**

- **`LZ()` 现在是通用的：占位符名 = `meta.learner[语言]` 里的键名转大写。**
  加一个键就多一个可用的占位符，某种语言用不上的键不用配，认不出的占位符
  **原样显示**（不再悄悄替换成空字符串）。这么改是因为波兰语有格变化：
  `{NAMA}`＝`Polska` 只能放主格，介词后面要属格，于是多了一个
  `{NAMA_GEN}`＝`Polski`（`jestem z Polski` / `wracam do Polski`）。
  `validate.js` 第 2 关会扫所有字符串：印尼语句子（所有界面语言共用）
  要求**每种语言都配齐**，学习者语言的句子只查它自己那一种。
- **`{PEOPLE}` 在波兰语里不能用。** `Polak / Polka` 分性别，读者可能是男是女。
  波兰语版一律改写成 `jestem z {NAMA_GEN}` —— 意思一样，男女都成立。
  同理，波兰语文案里**不要用带性别词尾的过去时**（`uczyłeś się`），
  一律写成现在时或无人称（`uczysz się` / `warto` / `można`）；
  实在避不开的第一人称过去时用 `Urodziłem/am się` 这种斜杠形式，
  书里人物说的话则跟人物性别走（Slamet 男、Shinta 女）。
- **波兰人和中国人踩的坑不是同一批，得逐条重写，不能直译。**
  一样是坑的：修饰语后置、kami / kita 不分、ini / itu 随说话人换位、复数靠重叠。
  反过来是便宜的：印尼语没有格 / 没有性 / 动词不变位（波兰语全都有）、
  两种语言都没有冠词、数字按位读、`setengah dua` 就是 `wpół do drugiej`。
  发音更是白送：`w`＝`ł`、`ny`＝`ń`、`kh`＝`ch`、`c`＝`cz`、`j`＝`dż`、
  `sy`＝`sz`、`y`＝`j`、`r` 同为颤音 —— 只剩词首 `ng` 和 `e` 的两个读法是真难点。
  这些都写进 `CLAUDE.md` 的内容规则了。
- **波兰语的数词要变格，界面文案不能赌词尾。** `1 strona / 2 strony / 5 stron`，
  `{0}` 是运行时才知道的。凡是数字会变的地方一律写成标签式
  （`stron: {0}`、`rozdziałów: {0} · stron: {1} · słów: {2}`）；
  只有 `review.count_n` 敢写 `{0} pytań` —— 因为可选值只有 10/15/20/25/30，
  全都吃属格复数。
- **地名要变格，人名交给译者自己判断。** 十个子代理里有的写 `w Jogja`、
  有的写 `w Yogyakarcie`。介词后面不变格的波兰语读着像句子没写完，
  回填后用脚本统一成 `w Jogji` / `w Yogyakarcie` / `z Lomboku` / `w Kudusie`（20 处）。
  人名（`payung Momoka` → `parasol Momoki`）保持译者的处理 —— 旁边就是印尼语原文，
  不影响对照。
- **全局 `button,input,select{appearance:none}` 会把 `<select>` 的箭头也吃掉。**
  改成下拉之后不补一个箭头，看着就是个方框，没人知道能点。
  现在 `.sel-ctl` 用内联 `data:` URI 画箭头（不发请求，`assertOffline` 照过），
  颜色取 `#928a80`，浅色深色两套底下都看得清。
  URL 里的 `//` 写成 CSS 转义 `\2f \2f `，免得将来有人给 `assertOffline`
  加一条更宽的正则时被误伤。
- **`.seg` 里的 `#langSeg` 没了，`speakables.js` 的渗漏检查要跟着改。**
  那段检查会把语言切换器整个摘掉再找中日文字符（「中文 / 日本語」不算渗漏），
  选择器写死的是 `#langSeg` —— 不改就会把五种语言的界面全报成渗漏。
- 加语言的顺序仍然是老规矩：**先抽出来翻译、回填，最后才改 `TRI_LANGS`**。
  `walkTri` 认的是「恰好这几个键」，先改判定它一个对象都认不出来。

---

## v1.6.0 做了什么（2026-09-02）

面向学习者的说明在 `CHANGELOG.md`，下面只记工程上要知道的。

| | |
| --- | --- |
| 来源 | Rae 拍的课本 Bab 3 照片 18 张（第 23–39、41 页；第 40 页没拍，应是写作用的空行页）。原片存 `inbox/done/L03/buku-bab3-foto/`（HEIC，gitignore） |
...
- **`<s>` 里别放数字和后缀。** `<s>pukul 09.00</s>`、`<s>-ku</s>`、`<s>Rp</s>`、`<s>Minggu / Ahad</s>` 各自生成了 "13.00"、"-ku"、"rp"、"Minggu Ahad" 这种没意义的音频条目。检查办法：跑 `npm run tts`（不带 `--yes`）看前几条，或按长度排序看最短的。
- **朗读文本里的数字写成印尼语单词**（`tanggal satu Agustus dua ribu dua`），不写阿拉伯数字 —— multilingual_v2 不传 language_code，数字可能被读成英文。旧内容里 `tanggal 3 Mei 2004` 这种已经有音频了，没动。
- **词条 `ex` 尽量照抄页面里已有的句子**（一字不差），哈希相同就不用再合成 —— 这次 69 个词只多花了约 700 字符。
- 第一人称「回国」用 `{NEGARA}`：`Tahun depan saya pulang ke {NEGARA}.` 四种界面各一条音频。
- 课本第 24 页的听力（Simak Audio 1/2）音频在 ugm.id 链接上，没收；只把题里的词（hadiah、liburan、menikah…）放进课文页的词卡。

---

## v1.5.0 做了什么（2026-09-02）
