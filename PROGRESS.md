...
- **表格优先照顾手机竖屏**，`min-width` 不要写死；标签列用 `width:1%` + `min-width`，
  **不能用 `nowrap`**（长标签会被撑开）。括号注释挪进表注。
- **横线能不画就不画。** 行间分隔用 `.row + .row{border-top}`，不要 `border-bottom`
  配 `:last-child`（这些行直接挂在页面上，`:last-child` 几乎不成立，
  于是块末尾留一条底线，再接 `rule` 就是两条并排）。`sec` / `mini` /
  带小标题的 `examples` 本身就是分隔，前面不用加横线。
...
- 课本 Bab 2 的听力题（音频挂在 ugm.id，外链不能内嵌）和成组的 Latihan 练习题
- 课本对齐还差三处需要新音频：`Sanggi dari mana?`（疑问词前后位置的对照）、
  听力词表的 `selamat pagi`
- APK 还没打过第一个
...
- 音频 708/708（100%），音库 2500 条，教材听力音轨 1 条
...
- [ ] 打第一个 APK（`npm run apk`，要 Rae 先确认过网页版）
- [ ] Bab 3 起：Rae 拍照放 `inbox/`，走 `/lesson`
- [ ] 补 Bab 2 的听力题和 Latihan（见上面「还没做的」）
