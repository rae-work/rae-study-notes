...
- **字体分工**：衬线只给内容（课文、对话、释义、标题）；
  界面元素（目录、按钮、菜单、输入框、筛选、数字）一律无衬线 —— 小字号衬线发糊。
...
- [ ] **补 21 个人名的语音** ← 卡在这里。第 10 页表格里的印尼人名刚标成可点读，
      但还没合成：21 条 / 96 字符 / 占月配额 0.24%。确认后跑 `npm run tts`。
      **在补齐之前 `npm run site` 会拒绝发布**（音频覆盖率不到 100%）
- [ ] **Rae 在手机上试用并确认**
