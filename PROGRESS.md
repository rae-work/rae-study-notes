...
| 引擎 | `qa_list` 的题目可带 `full`（印尼语侧）→ 显示成「保底版 / 完整版」两档；点哪档的 ▶ 只揭开哪档。新增 `block.qa_short` / `block.qa_full` 五语键；`validate.js` 第 1 关检查 `full` 结构 |
| 第三课 | 讲义 1b–4b 的 40 题补完整版；新增第 26 页「Latihan Kalimat 3」（1a–4a 各 12 行示范表）；原「课本还剩一题」note 改为指向新页。L03 变 28 页 |
| 词汇 / 题库 | +84 词（`les: 4`，例句取课内句子）；`situasi` +6（家庭问答） |
| 语音 | 计划 648 条 / 16,773 字符，账号剩 27,867 —— **等 Rae 确认再跑** |
