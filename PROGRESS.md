...
| Bab 2 | 新建 `L02.json`：一页词汇（26 词）+ 两条笔记（领属语序、-ku/-mu 是口语）。课文和练习等材料整理完再补 |

**等 Rae 决定**

- **78 条朗读文本还没有音频（940 字符，约剩余额度的 4.3%）** —— 她说过不要花额度，
  所以现在走系统语音。跑一次 `npm run tts` 就能补上。**不补的话 `npm run site` 和
  `npm run apk` 会被拦**（`assertNoMissing()`），只有本地预览和 build 不受影响。
  ⚠️ 不补的代价比想象中大：安卓的 TTS 桥接（`MainActivity.java:102`）在没装印尼语
  语音包时静默 return、从不回调，于是答对之后要等满 `AUTO_MAX_HOLD`（8 秒）
  才自动翻页 —— 好在现在每屏都有「继续」，用户可以自己走。
