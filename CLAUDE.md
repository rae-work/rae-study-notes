...
- 学习者语言：`zh`（主）、`ja`、`en`、`vi`、`pl`。所有面向学习者的文字（释义、说明、笔记、界面文案）都是 `{"zh":…, "ja":…, "en":…, "vi":…, "pl":…}` 对象；印尼语始终是纯字符串。键的名单在 `scripts/lib/content.js` 的 `TRI_LANGS`。
- **哪几种语言是硬性要求，看 `content/meta.json` 的 `required_langs`。**
  **目前是 `["zh","ja","en","vi","pl"]` —— 五语全必填，不齐不许构建。**
  （以 meta.json 为准；任何文档里写着 `["zh"]` 或「三语 / 四语」的都是旧的。）
- **加一门新语言的顺序不能反**：先用旧的键数判定把内容抽出来翻译、回填，
  最后才把新语言加进 `TRI_LANGS`。`walkTri` 认的是「恰好这几个键」——
  先改判定，它一个对象都认不出来，校验会显示「0 个对象」而不是报错。
- **印尼语里「中国」一律写 `Tiongkok`，不写 `Cina`**（Rae 2026-09-13 定的，印尼官方规范用词）。只改印尼语文本；学习者语言里的「中国 / China / Trung Quốc …」不动。
...
- 波兰语同样要按波兰语的事实重写，不是从中文直译。**它跟中文站在哪边，逐条不一样**：
  - **一样是坑**：修饰语后置（`buku saya` / `rumah besar`，波兰语形容词和物主代词都在前）、
    kami / kita 不分（波兰语只有 `my`）、ini / itu 随说话人换位、复数靠重叠而且常常不标。
  - **反过来是便宜**：印尼语没有格、没有性、动词不变位 —— 对波兰人是最省力的地方；
    两种语言都没有冠词；数字按位读法一样；`setengah dua` 和 `wpół do drugiej` 完全一致。
  - **发音基本白送**：`w`＝`ł`、`ny`＝`ń`、`kh`＝`ch`、`c`＝`cz`、`j`＝`dż`、
    `sy`＝`sz`、`y`＝`j`、`r` 同为颤音。只剩词首 `ng` 和 `e` 的两个读法是真难点。
  - 语体 `Anda`／`Bapak-Ibu` ↔ `kamu` 用 `Pan / Pani` ↔ `ty` 解释，比中文的「您／你」更贴切。
  - 引号用 „ ”，称呼读者用非正式第二人称，**动词要挑不带性别的形式**
    （`uczysz się`／`warto`，不要 `uczyłeś się`）—— 读者可能是男是女。
  - 单复数：波兰语数词后面要变格（1 strona / 2 strony / 5 stron），界面文案里
    凡是数字会变的地方一律写成 `stron: {0}` 这种标签式，别赌某一个词尾。
- 中文、日文句内引号用「」；越南语用 “ ”；波兰语用 „ ”。
- **课文、对话、语法例句一律仿写，不照抄课本**（Rae 2026-08-25 定的）。
  仓库和网站都是公开的，把整章誊清等于做了一份可替代原书的副本。
  做法：课本的**场景、语法点、固定表达、词形**照收（那些是语言事实），
  **句子自己写** —— 同样的教学目的、不同的句子。人名地名、年龄职业等事实跟课本一致，
  这样课本的问题照样能用改写后的课文回答。
- **例外：课本和讲义上的练习题，从 Bab 4 起用原句、按原题号收**（Rae 2026-09-13 定的）。
  原因是课堂会随机点人做题，答案必须跟课本同一题一一对上才有用。
  每题**只写一句完整答案**，用 `answer.hl` 把「被点名时最少要说的部分」加粗 ——
  只念粗体也必须是通顺、正确的回答。
  **只跟某一题有关的提醒**（陷阱、老师看着你说时怎么答、化解尴尬的一句）写进该题的 `tip`，显示在答案正下方（细竖线小字）——
  不要堆在页面底部的 note 里，读到那一题时根本看不到（Rae 2026-09-13）。底部 note 只放整组题通用的规则。
  **填空题不写 `prompt`**：整句直接露出，要填的词写进 `hl`（加粗，遮挡模式只遮这几个词），不要再列一遍带 ___ 的题干。**不要写两遍**（Rae 试过保底版 + 完整版两行，
  嫌重复、上课紧张时找不到）。练习题目那一侧 `kw` 留空，页面上唯一的粗体就是保底部分。
  Bab 1–3 的练习页是按旧规矩仿写的，没有改回原句。
- 讲义几乎每次都有笔误：**静默改正内容，页面上不加「笔误修正」说明**（Rae 2026-09-13 定的：对读者没帮助、占版面；所有旧的笔误 note 已删）。改了哪些只记在 `PROGRESS.md`，给 Rae 的摘要里说一声。同时主动补课件没讲但一定会踩的坑（例：bisa vs boleh、harus 的否定是 gak usah / tidak perlu、Minggu 既是周日也是星期）。
...
- **例句里「学习者自己」的国籍写占位符**，跟着界面语言变：
  ...
  映射表在 `meta.json` 的 `learner`，**占位符名就是那里的键名转大写** ——
  加一个键就多一个可用的占位符，某种语言用不上的键不用配。
  波兰语因此多了 `{NAMA_GEN}`（属格：`z Polski` / `do Polski`），
  而且**不用 `{PEOPLE}`** —— `Polak / Polka` 分性别，一律改写成
  `jestem z {NAMA_GEN}`，男女都成立。
  认不出的占位符会原样显示，`npm run validate` 第 2 关会拦下来。
  **只用于第一人称** —— 讲课本人物的例句不动。改了映射必须重跑 `npm run tts`。
- 学习者文本里可以用 `<b>` `<i>` 强调、**`<s>词</s>` 把印尼语标成可点读**
  （表格单元格里同样有效）。其余按原样输出。
- 界面上任何文案都要走 `T()`，键要同时加进 `ui.zh/ja/en/vi/pl.json` 五个文件，
  **键集合必须完全一致**，少一个就构建不过。
- 朗读文本保留 `?` `!`，问句要按问句语调合成。
...
## 每次改动后必跑
`npm run validate` → `npm run build`。校验内容：JSON 结构、五语齐全、占位符自洽、语体值合法、配对可解析、词汇查重、`node --check`、jsdom 逐块渲染冒烟、实战题库压力测试（选项恰好 4 个且不重复、答案索引正确）、朗读文本的音频覆盖率。前 8 关不过会直接失败；**第 9 关（音频覆盖率）只是警告** ——
