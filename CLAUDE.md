...
- **哪几种语言是硬性要求，看 `content/meta.json` 的 `required_langs`。**
  **目前是 `["zh","ja","en"]` —— 三语全必填，不齐不许构建。**
  （以 meta.json 为准；任何文档里写着 `["zh"]` 的都是旧的。）
- 中文、日文句内引号用「」。
...
- **例句里「学习者自己」的国籍写占位符**，跟着界面语言变：
  `{NEGARA}`（印尼语国名，**会进朗读文本，三种语言各产生一句、都要有音频**）、
...
- 学习者文本里可以用 `<b>` `<i>` 强调、**`<s>词</s>` 把印尼语标成可点读**
  （表格单元格里同样有效）。其余按原样输出。
- 界面上任何文案都要走 `T()`，键要同时加进 `ui.zh/ja/en.json` 三个文件，
  **键集合必须完全一致**，少一个就构建不过。
- 朗读文本保留 `?` `!`，问句要按问句语调合成。
...
## 每次改动后必跑
`npm run validate` → `npm run build`。校验内容：JSON 结构、三语齐全、语体值合法、配对可解析、词汇查重、`node --check`、jsdom 逐块渲染冒烟、实战题库压力测试（选项恰好 4 个且不重复、答案索引正确）、朗读文本的音频覆盖率。前 8 关不过会直接失败；**第 9 关（音频覆盖率）只是警告** ——
