...
- **每页第一块必须是 `phead`，且必须带 `cat`。** 目录按它分组、眉标按它显示，取值只能是：
  `intro` 本章概览（第一页）· `bunyi` 发音 · `kosakata` 词汇与表达（词表、固定说法、疑问词、数字、日期）·
  `tatabahasa` 语法（含课堂语法笔记）· `percakapan` 对话与课文（对话、听力、阅读）· `latihan` 练习 · `wawasan` 印尼常识。
  名字在 `ui.*.json` 的 `cat.*`，校验的名单在 `scripts/validate.js` 的 `PAGE_CATS`，引擎的顺序在 `CAT_ORDER`。
- 目录里显示的是 `sub`（界面语言），印尼语 `title` 退成小字 —— 所以 `sub` 要写成学习者一眼能认的名字；
  开头如果就是分类名（「语法 · 人称代词」），目录会自动去掉那一截。
- 每页一到两屏，宁可多分页。
...
| `k` | 用途 | 字段 |
|---|---|---|
| `phead` | 页标题（每页第一个） | `title`（印尼语）· `sub`（多语）· **`cat`（页面分类，必填）** |
| `lead` | 页首导语 | `text`（多语） |
| `psub2` | 小段说明 | `text`（多语） |
...
| `qa_list` | 问答 / 翻译练习 | 可选 `title` · `items: [{prompt, answer}]`，answer 自动可遮挡；印尼语答案可带 `hl: ["片段", …]`，把「被点名时最少要说的部分」加粗（按顺序查找，不分大小写）；**不写 `prompt` 就是填空题**：整句露出，只遮 `hl` 那几个词（空格），不要再把带 ___ 的题干重复一遍；**只跟某一题有关的提醒写进该题的 `tip`（多语，可用 `<b>` `<s>`）**，显示在答案正下方，不要堆到页面底部的 note 里 |
