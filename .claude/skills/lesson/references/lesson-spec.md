...
| `qa_list` | 问答 / 翻译练习 | 可选 `title` · `items: [{prompt, answer}]`，answer 自动可遮挡；印尼语答案可带 `hl: ["片段", …]`，把「被点名时最少要说的部分」加粗（按顺序查找，不分大小写）；**不写 `prompt` 就是填空题**：整句露出，只遮 `hl` 那几个词（空格），不要再把带 ___ 的题干重复一遍 |
